...
/// @title FXRPVault
/// @notice MVP de hackathon: depositar FXRP, ver valor en USD en vivo via FTSOv2,
...
contract FXRPVault {
    ...
    /// @param _fxrpToken Dirección del token FXRP en Coston2.
    /// IMPORTANTE: no la inventes. Sacala de la documentación oficial de FAssets
    /// (dev.flare.network/fassets/overview) o del contrato AssetManager en testnet
    /// antes de deployar — esa dirección cambia entre redes y versiones.
    ...
    /// @dev Ajustá el divisor `1e6` a los decimales reales del token FXRP (verificalos).
        ...
        returns (uint256 usdValue, int8 priceDecimals, uint64 timestamp)
    {
        ...
        usdValue = (bal * price) / 1e6; // TODO: reemplazar 1e6 por 10**decimalesFXRP
        priceDecimals = dec;
        timestamp = ts;
    }
